...
name: reviewer
description: |
  Code review from a fresh, independent context. Use when you want a second
  opinion on a diff, a plan, or a proposed approach. Also use for general
  codebase health audits and PR/issue validation. Prefer reviewer over
  self-review whenever the change is non-trivial — it inspects from scratch
  without the biases of whatever wrote the code. Can also apply small fixes
