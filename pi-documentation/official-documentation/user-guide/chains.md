...
When run through Agent Deck, a chain becomes an app-owned graph run. Each step is tracked as a child node with status, transcript, artifact path, optional worktree path, summary/error, and duration.
...
Older docs may mention chain files inside `.agents/`. The current Agent Deck scanner does not actively discover legacy `.agents` chain files as managed chains.
