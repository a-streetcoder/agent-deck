...
/**
 * Effect Schema port of the WebSocket wire contract (source of truth today:
 * `packages/domain/src/protocol.ts`, zod). The wire format is IDENTICAL — this
 * package moves the schema technology, not the bytes. A golden-fixture parity
 * test (test/protocol-parity.test.ts) asserts both validators accept/reject
 * the same corpus.
 *
 * Arrays and records are wrapped in `Schema.mutable` so the derived types match
 * the zod-inferred / hand-written interface types in `@agent-deck/domain`
...
export const ClientMessage = Schema.Union(
  ...
  Schema.Struct({
    ...
    response: Schema.mutable(Schema.Record({ key: Schema.String, value: Schema.Unknown })),
  }),
);
...
export const SessionPlanItem = Schema.Struct({
  id: Schema.String,
  title: Schema.String,
